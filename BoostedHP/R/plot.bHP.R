...
#' @param plot_type a character string specifies the style of plot.'static' for static figure, 'JS' for plotly.js, a web-based interactive charting library, 'dynamic' for dynamic figure showing history of iterated process.
#' @param interval_t a positive number to set the time interval of the animation (unit in seconds); default to be 0.3.
#' @param ylab a title for the y axis: see 'title' in package 'graphics'.
#' @param col_raw A specification for the default plotting color of raw data. See section ‘Color Specification’.
#' @param col_trend_h A specification for the default plotting color of trend history. See section ‘Color Specification’.
#' @param col_trend_f A specification for the default plotting color of final trend component. See section ‘Color Specification’.
#' @param col_pvalue_BIC A specification for the default plotting color of p-value of BIC. See section ‘Color Specification’.
#' @param raw_alpha a numeric vector from 0 to 255 modifing colour transparency of plotting raw data. The smaller the number, the more transparent.
#' @param trend_h_alpha a numeric vector from 0 to 255 modifing colour transparency of plotting trend history. The smaller the number, the more transparent.
#' @param trend_f_alpha a numeric vector from 0 to 255 modifing colour transparency of plotting final trend component. The smaller the number, the more transparent.
#' @param pvalue_BIC_alpha a numeric vector from 0 to 255 modifing colour transparency of plotting p-value or BIC. The smaller the number, the more transparent.
#' @param legend_location a character string or a pair of numeric vector specifing the location of legend. The choice set of the character sting are: upleft, downleft, upright, downright.
#' @param iteration_location a character string or a pair of numeric vector specifing the location of 'iteration time'. The choice set of the character sting are: upleft, downleft, upright, downright.
#' @param cex_text The magnification to be used for showing 'iteration time' relative to the current setting of cex.
#' @param cex_legend The magnification to be used for legend relative to the current setting of cex.
...
plot.bHP <- function(x, plot_type= "dynamic", interval_t = 0.3,  ylab = "",
  ...
  if(plot_type == "JS"){
    ...
    p1 <- plotly::plot_ly(data_1, x = ~date, color = I(col_raw)) %>%
      plotly::add_lines(y = ~raw_data,name = "raw data", legendgroup = "raw data") %>%
      plotly::add_lines(y = ~trend_data, color = I(col_trend_f),name = "final trend", legendgroup = "final trend") %>%
      plotly::layout(showlegend = T, yaxis = list(title = ylab),title = main)
    ...
    if(x$test_type == "adf"){
      ...
      p2 <- plotly::plot_ly(data_2, x = ~iter_time, y = ~p_value,color = I(col_pvalue_BIC),
                    name = "p-value", legendgroup = "p-value")

      return(plotly::subplot(p1, p2, heights = c(0.7,0.3), nrows = 2, margin = 0.05))
    }
    ...
    if(x$test_type == "BIC"){
      ...
      p2 <- plotly::plot_ly(data_2, x = ~iter_time, y = ~BIC,color = I(col_pvalue_BIC),
